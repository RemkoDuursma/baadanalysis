...
\item For the aboveground biomass pools, we calculated the leaf mass fraction ($M_F / M_W$) and leaf area ratio ($A_F / M_W$). LMF, LAR, SLM, $M_F$ and $A_F$ are related by,

$$LMF = SLM * LAR = (M_F / A_F) * LAR$$
...
\item We do note that there was substantial variation between individual plants (Figs.~\ref{fig:figureSI3} and~\ref{fig:figureSI3}). We estimated the variance explained by PFT, plant height, and biome in $M_F/A_S$ using a linear mixed-effects model (see Table~\ref{varpartpipe}). We found that plant height and biome explained only \Sexpr{100*signif(Table_pipemodel_varpart$Marginal[2],2)}\%, but when PFT was added to the model, the variance explained increased to \Sexpr{100*signif(Table_pipemodel_varpart$Marginal[3],2)}\%. If we used SLM as a predictor instead of PFT, the $R^2$ was similar (\Sexpr{100*signif(Table_pipemodel_varpart$Marginal[4],2)}\%), showing that PFT affected $M_F/A_S$ largely through differences in SLM. Consistent with this, PFT, plant height and biome explained very little variance in $A_F/A_S$ (Table~\ref{varpartpipe}).

\item We found similar results for the above-ground leaf mass fraction (LMF), where a clear and strong separation was found between plant functional types (Fig.~\ref{fig:figure3}, see also Fig.~\ref{fig:figureSI5}). For the leaf area ratio (LAR), PFTs were generally closer together, with a notable exception for small evergreen gymnosperms (< 1m height) (Fig.~\ref{fig:figure4}). Calculated least-square means of LMF and LAR (averages accounting for variation explained by plant height) showed very similar patterns: large differences in mass-based allocation between PFTs (LMF), and small differences in area-based allocation (LAR) (Fig.~\ref{fig:figure5}).
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.99\textwidth]{figure5_lsmeans_LMFLAR.pdf}
    \caption{Average leaf mass fraction (LMF) and leaf area ratio (LAR) accounting for variation explained by plant height (least-square means) with a mixed effects model (with species within study as a random effect). Error bars are 95\% confidence intervals.}
    \label{fig:figure5}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.99\textwidth]{figureSI-8_MAPMAT_vegetation.pdf}
