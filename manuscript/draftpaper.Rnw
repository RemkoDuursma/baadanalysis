...
% Notes from Daniel phone call
% - make case that allocating biomass vs. targeting certain leaf area vs. stem area is very different
% 
% fig 1 : more grey levels. scale for grey levels
% figure captions need to be stronger statements
% 
% use all data with leaf mass, explain that subset has same results
% 
% Fig 9 + 10 instead of Fig 2 in main text.
% Then, Fig 3 says that SLM explains differences.
% 
% Fig 3 + 6 log-log axes
% 
% Fig 4 first, introduce PFT differences
% 
% No difference in m.so/a.astba2, but large difference m.lf/a.stba2
% 
% remove fig 5
% 
% combine fig 3
% 
% climate-related effects are captured by shift in PFT
% 
...
\begin{enumerate}
\item The importance of biomass allocation in global vegetation models \cite{ise_comparison_2010}, feedback on fluxes of CO2, lifetime of C in the system \cite{friend_carbon_2014}, which depends on partitioning of biomass into short-lived foliage vs. long-lived woody pools.
...
\item For the aboveground biomass pools, we calculated the leaf mass fraction ($M_F / M_T$, where $M_T$ is total aboveground biomass) and leaf area ratio ($A_F / M_T$). LMF, LAR, SLM, $M_F$ and $A_F$ are related by,

$$LMF = M_F / M_T = (M_F / A_F) (A_F / M_T) = SLM * LAR$$

\item We can further decompose aboveground biomass ($M_T$) into a size-related metric (basal stem area, $A_S$), and the ratio of $M_T$ to $A_S$, which is a function of wood density and plant woody volume to basal area. 
...
\item We used a generalized additive model (GAM) to visualize the relationships, but these were not used for formal inference.

\item We used linear mixed-effects models (with species within study as the random effect) to test for significance and to calculate confidence intervals for PFT-specific averages. Least-square means for LMF and LAR were calculated with the 'lsmeans' package.

\item All variables were log-transformed prior to analysis, to stabilize variance.

\item Variance partitioning with marginal (fixed effects) and conditional (fixed and random effects) $R^2$ for linear mixed-effects models as reported by \cite{nakagawa_general_2013}.
...
\item A recent study compared allocation routines in a number of leading ecosystem models \cite{de2014does}, and recommended to constrain allocation by observed biomass fractions instead of using constant allocation fractions, which is common practice.
...
Reich et al 1992, for seedlings, negative relationship leaf lifespan and LAR. This is different from our results, since SLM is nearly proportional to leaf lifespan, and we found no relationship between LAR and SLM (but do they have all the PFTs??). However, across stands, positive relationship between leaf biomass and leaf lifespan, which is consistent with our results. 

Cornelissen et al 1996 positive relationship SLA and LAR, but this is for tiny seedlings harvested 21 days after first leaf appearance. But check references and who cites this!
