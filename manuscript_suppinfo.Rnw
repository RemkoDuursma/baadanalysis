...
\begin{figure}[h!]
    ...
    \caption{Leaf area ratio ($\afmt$) by plant height (H) and PFT. Each symbol is an individual plant. Lines are generalized additive model fits using species/dataset as a random effect. Shaded areas are approximate 95\% confidence intervals.}
    \label{fig:figureS2}
\end{figure}
