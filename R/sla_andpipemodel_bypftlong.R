...
meansbypftlong <- function(yvar1, yvar2, xlab=expression(Specific~leaf~area~(m^2~kg^-1)), 
                           ylab1=NULL, ylab2=NULL, addtrend=c(TRUE,FALSE),
  ...
  f1 <- lmer(Y1 ~ pftlong - 1 + (1|Group), data=dat)
  f2 <- lmer(Y2 ~ pftlong - 1 + (1|Group), data=dat)
...
to.pdf(
  meansbypftlong("lmlf_astbh","lalf_astbh", ylab2=expression(A[L]/A[Sbh]~~(m^2~m^-2)),
