...
These patterns in biomass distribution reflected differences in the distribution of components of eq. \ref{eq:mfmspart}-\ref{eq:afmspart}. In particular, LMA was found to differ substantially among PFTs, whereas $\afas$ did not (Fig. 6), which is consistent with the large differences between PFTs in LMA shown in Fig. 3b,c.  When PFT was added to the statistical model (as an intercept term as well as interaction with the smooth plant height term), the $R^2$ increased substantially for $\mfaf$ but not $\afas$  (Table \ref{table:varparttable}), demonstrating that PFT differences entered largely in leaf mass-based variables.
