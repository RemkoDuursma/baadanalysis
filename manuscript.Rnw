...
\begin{itemize}
\item Dataset covers climate space pretty well. Evergreen angiosperms occurred at higher MAT than deciduous angiosperms and evergreen gymnosperms but there is much overlap between PFTs. 
...
\item Partitioning MF/MS into three component variables, and partition each of the four into H, PFT and climate. Climate effects, when analyzed in addition to H and PFT, were very weak, with only LMA and AF/AS somewhat more dependent on climate, but not consistently (Fig S4).
\item We show that AF was proportional to AS (with the exception of very large trees). AF/AS was nearly unrelated to PFT or plant height.
...
\item We repeat an analysis similar to Reich et al 2014 to test for climate effects on biomass partitioning when only considering the angiosperm/gymnosperm divide, and not first correcting for H as we did previously (but instead binning by H). We find that for angiosperms, MF/MS increased with MAT for all but the smallest plants. But not for gymnosperms (Fig. 7). 
\end{itemize}
...
\begin{figure}[h!]
    ...
    \fi
    \caption{Relationships between leaf - stem biomass distribution ($M_F/M_S$) and mean annual temperature (MAT) (degree*C). Similar to Reich et al. (2014), we divided the data into five bins of plant size (bins were chosen so that each bin had equal sample size), and used only evergreen vs. deciduous plant functional types. Lines are fits from mixed-effects models (with species within study as random effect, and only MAT and evergreen/deciduous classification as fixed effects). Solid lines are significant (p < 0.05), dashed lines are not (p > 0.05). Symbols are partial residuals as calculated with the visreg package in R.}
