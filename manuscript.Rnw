...
\item Our analysis of climate effects on above-ground biomass distribution between leaves and stems is largely consistent with Reich et al. 2014. We found that for angiosperms, MF/MS increased with MAT (as did Reich), after splitting the dataset into five equally populated bins. This result is in apparent contradiction with our finding that climate (MAT, MAP and its interaction) explained consistently very little variation in MF/MS. In that analysis, however, the effects of climate were analyzed in addition to the potentially confounding factors plant height and plant functional type. The significant relationship between MF/MS and MAT for angiosperms can be understood by the consistently higher proportion of evergreen angiosperms at higher MAT (Fig. 1), which, as a resutlt of higher LMA (Fig. 2) also have a higher MF/MS. The main conclusions from the climate analysis are certainly that PFT and plant height are by far the most important factors, this is also consistent with Reich 2014 and Poorter 2015. 
...
\begin{figure}[h!]
    ...
    \fi
    \caption{Climate effects as per Reich et al. (2014).}
