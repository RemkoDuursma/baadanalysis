...
\item AF vs. AS by pft, for all data, with fitted SMA. This shows AF is proportional to AS (approx, except larger plants), and no/very very small effect of PFT.
\item Stem mass as function of AS, PFT and height. a) Raw data (to link to main equation) show no clear PFT effect. b) MS / AS is nearly proportional to H, as expected from simple stem mass equation. c) for larger plants (H*AS > 0.01m3), there is a significant PFT effect, with EG having lower MS / (AS*H) (see d) for averages across entire subset shown in c). This can be explained from differences in wood density; gymnosperms have lower wood density on average.
\item Histograms of the three main variables: AF/AS, MF/AF, and height-corrected stem mass index, MS/(AS*H).
\end{enumerate}
...
\item Enquist&Niklas showed differences in leaf mass at given stem mass between gymnosperms and angiosperms.Poorter2015 showed X. Reich2014 showed Y. Open questions are 1,2,3. One major gap is that these previous studies did not include estimates of whole-plant leaf area. Leaf area is relevant because bla, but may follow very different patterns from leaf mass. Poorter2015 speculated that X.
\item What about climate? We know from small-scale experiments and observational studies that biomass distribution varies with climate drivers. But unclear if global patterns persist. Reich2014 showed X, consistent with glasshouse stuff from Poorter2015. 
\item We have the following objectives:
\begin{enumerate}
\item Test whether MF/MS is proportional to leaf mass per area (as suggested by Poorter)
\item Partition the variance of leaf/stem biomass distribution into component variables, and into plant height (which is important, obviously!), plant functional type and climate. 
\end{enumerate}
