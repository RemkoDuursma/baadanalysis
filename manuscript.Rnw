...
\title{Leaf traits drive differences in above-ground biomass distribution among major plant functional types}
...
\begin{enumerate}
\item Reduce focus on GVMs in introduction (Reviewer 2).
\item Better linking to Poorter 2015 and Reich 2014 (editor, reviewer 2, reviewer 3, poorter pers comm).
\item Remove LMF and LAR, and only analyze MF/MS and AF/MS. This avoids confusion of the terms LMF and LAR which normally include root biomass. We should more clearly focus on above-ground biomass distribution patterns (editor + reviewer 2).
\item Partition variance in MF/MS directly, not the proxy variable MF/AS. We did that for statistical convenience, as reviewer 1 is correct in pointing out. But we can look at partitioning variance in MF/MS directly, to plant height, PFT and climate. Reviewer 3 (though incorrectly) says 'also have the same variable in the numerator and denominator, which causes statistical issues' - but then we can say we avoid this apparent problem.
\item We claim that climate does not affect biomass distribution, but reviewer 1 correctly points out that climate affects plant height, so it may have an indirect effect. Our dataset is not appropriate for testing effects of climate on typical (or maximum) plant height, because individuals of all kinds of sizes were sampled; there will be sampling bias. Moles et al. 2009 reported that maximum plant height shows a strong latitudinal pattern (taller plants at low latitude), and is correlated with climate variables. We just have to describe their results.
\item Can follow reviewer 1's suggestion to partition MF/MS like this: $\frac{M_F}{M_S} = \frac{A_F}{A_S}\frac{M_F}{A_F}\frac{A_S}{M_S}$
\item Reviewer 2 has some strange comments about MS/AS, in that we expect them to be correlated. Yes, but we need to make the point more clearly that there could be PFT effects in this correlation, largely through wood density (and tree shape). In fact we bring this up, but partitioning the variance as above may make this stand out more.
\item "The methods do not describe if the individual data points came from trees in closed-canopy or open stands.  That would also make a huge difference." (Reviewer 3)
\item We used MAT, MAP as well as the interaction. Probably better climate variables out there, but having the interaction in a model does distinguish between warm-dry and warm-wet climates (just like aridity indices). Because we found no appreciable effects of climate, we did not think it was worthwhile testing all possible climate variables.
\end{enumerate}

\section{New list of figures}
\begin{enumerate}
\item Tree diagram plus climate space of dataset. Unchanged from before, except remove MT (?).
\item MF and MS as function of tree height. Reviewer 2 asks "But are differences in Figure 2 statistically signficant?", weird question and subsequent comments. Figure could add trend lines (as smoothers), but refer to later figures for significant differences. The comment that sampling may have been uneven is correct, which is why we correct for plant height in the analyses.
\item (a) MF/MS vs. plant height by PFT (was LMF) (b) and (c) as before, except for MF/MS. Reviewer 2 wants us to remove panels d and e, which we can do because we won't need MF/AS as a proxy anymore (should be put in SuppInfo instead).
\item Histograms by PFT. This was nice when we used size-invariant MF/AS, but we can still show LMA and AF/AS? The first has large PFT differences, the second not any.
\item Not sure how current Fig 5 will fit in the new version
\item Not sure how current Fig 6 will fit in the new version, since we won't be able to use MF/AS. Can make this plot for AF/AS though, since reviewer 1 thinks that has biological relevance (and MF/AS doesn't, for some reason).
\end{enumerate}
...
% 200 word max.
\begin{enumerate}
\item The partitioning of biomass into leaves and stems is an important component of  global vegetation models (GVMs). Although GVMs typically assume that the major woody plant functional types (PFTs) differ in biomass partitioning, empirical studies have not been able to justify these differences. Here we test for differences between PFTs in distribution of biomass between leaves and stems.
...
\item The leaf mass fraction (LMF, leaf / total aboveground biomass) varied strongly between PFTs (as defined by deciduous vs. evergreen and gymnosperm vs. angiosperm). We found that LMF, once corrected for plant height, was proportional to leaf mass per area across PFTs. As a result, the PFTs did not differ in the amount of leaf area supported per unit above ground biomass. We found only weak and inconsistent effects of climate on biomass partitioning.
\item Combined, these results will allow for systematic benchmarking of biomass partitioning routines in GVMs.
\end{enumerate}
...
% no longer needed? we will use biomass distribution instead of partitioning to be consistent with Poorter 2015.
...
where $M_i$ is the biomass in some compartment (leaves, stems or roots) remaining on the plant, $a_i$ the annual allocation of NPP to that compartment, and $\tau_i$ the annual turnover (or loss) of compartment $i$ from the plant. It is thus easy to see that partitioning ($M_i/M_T$, where $M_T$ is total biomass) can be different from allocation ($a_i$) because turnover ($\tau_i$) differs between leaf and wood biomass. Here, we present data on biomass partitioning, which can inform models of allocation only when estimates of turnover are available.
...
% we used basal area because many plants were <1.3m tall
...
Despite the exceptional size of our dataset, the strong size-dependence of LMF still hinders comparisons across climatic gradients, due to the small sample size available within each species or site when sampling at a common height. To study climate effects on biomass partitioning, we therefore further decomposed LMF as the product,

\begin{equation}\label{eq:M1}
\textrm{LMF} = \left(\frac{M_F}{A_S}\right) \left(\frac{A_S}{M_T}\right).
\end{equation}

This decomposition showed that the vast majority of size-related variation was captured by $A_S/M_T$ alone, indicating that comparisons among $M_F/A_S$ could be made without needing to compare at a common height. A similar decomposition  of LAR as the product of $A_F/A_S$ and $A_S/M_T$ produced the same outcome. We therefore analysed for climatic effects on $M_F/A_S$ and $A_F/A_S$ in two ways: with quantitative variables (mean annual precipitation, MAP; mean annual temperature, MAT), and by biome (tropical / temperate/ boreal), a simple classification taking into account both MAT and MAP (Fig. S1). In both cases we used PFT and plant height as covariates varying across climate space. 

All analyses were conducted in R v3.2.0 \citep{R-2015}. GAMs were fitted using \emph{mgcv} package \citep{wood2006}. The code to replicate this analysis are available on GitHub at http://github.com/RemkoDuursma/baadanalysis.

\section*{Results}

The raw data in Fig.~\ref{fig:figrawdata} show a steeper increase of woody aboveground biomass ($M_S$) with plant height compared to foliage biomass ($M_F$). As a result, LMF decreased with plant height, with the three plant functional types clearly differing in LMF across nearly the entire size range (Fig.~\ref{fig:figlmf}a). When we corrected for plant height by estimating LMF at a common plant height, we found that LMF was proportional to the average leaf mass per area (LMA) across the three PFTs (Fig.~\ref{fig:figlmf}b). As a consequence, LAR was invariant between PFTs, because LAR = LMF / LMA (Fig. ~\ref{fig:figlmf}c and Fig. S2). 

Mirroring the results for LMF, we found that the amount of leaf mass per unit stem area ($M_F/A_S$) differed among the three PFTs, that these differences were also proportional to LMA (Fig. ~\ref{fig:figlmf}d), and that PFTs were similarly invariant in the amount of leaf area per unit stem area ($A_F/A_S$) (Fig.~\ref{fig:figlmf}e).

The above results highlight how LMA drives differences in biomass partitioning among dominant woody PFTs. To assess whether LMA could replace PFT when predicting biomass partitioning, we fit a linear mixed-effects model to LMF and $M_F/A_S$ using PFT and plant height (and the quadratic term) as predictors (and all interactions). We then replaced PFT with LMA, and found that the model with LMA could explain almost as much variation in LMF as the model with PFT ($R^2$ = 0.74 with PFT vs. 0.62 with LMA), likewise for $M_F/A_S$ ($R^2$ = 0.29 vs. 0.28). 

There was considerable variation in all studied variables between species within PFTs (Fig. ~\ref{fig:fighist}). To understand whether we can improve on a PFT-based classification by including other traits that affect biomass partitioning, we decomposed $M_F/A_S$ into LMA ($M_F/A_F$), and the ratio of leaf area to stem cross-sectional area ($A_F/A_S$):
...
We found that decomposing LMF and LAR as shown in Eq. \ref{eq:M1} was very useful to study climate effects because the second term ($A_S/M_T$) absorbed nearly all of the size-related variation in LMF (Table \ref{table:statstable} and Fig. S3) and is otherwise fairly conserved across PFTs (Fig. S3). As shown in Fig.~\ref{fig:figlmfspecies}b and Fig.~\ref{fig:figlmf}d, the term $M_F/A_S$ exhibits comparable differences between PFTs to LMF, but unlike LMF, $M_F/A_S$ is nearly independent of plant height (Table \ref{table:statstable}). $M_F/A_S$ is thus a useful proxy for LMF that can be compared across species and sites. 

Regardless of how we analysed the data, we found only weak climatic effects on biomass partitioning within PFTs (Fig.~\ref{fig:figclimate}). Biome consistently explained very little variation when added to a statistical model in addition to PFT and plant height ($R^2$ increased by only 0.01 - 0.06, see Table \ref{table:statstable}). Likewise, we found weak and inconsistent effects of climate variables (MAP and MAT) (Fig.~\ref{fig:figclimate}b-c). These variables explained little variation when added to a statistical model in addition to PFT and plant height ($R^2$ increased by 0.01 and 0.11, respectively, see Table \ref{table:statstable}).
...
%'       hline.after=0:16, caption.placement="top",  #c(0,4,8,12,16)
