...
\item Climate variables (MAT, MAP and its interaction) consistently explained very little additional variation, demonstrating very weak control of climate on biomass distribution and leaf area, at a given plant size. We argue that climate effects will largely be indirectly through effects of plant height.
...
\item Partitioning MF/MS into three component variables, and partition each of the four into H, PFT and climate. Climate effects were very weak, with only LMA and AF/AS somewhat more dependent on climate, but not consistently (Fig S4).
