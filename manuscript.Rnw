...
All analyses were conducted in R 3.2.3 \citep{R-2015}, using the packages `mgcv` \citep{wood2006}, `hier.part` \citep{walsh2013}, `MuMIn` \citep{barton2015}, re and `remake` \citep{fitzjohn2015}. In addition, the code replicating this analysis (and all figures) is available at \url{http://github.com/RemkoDuursma/baadanalysis} (doi: \emph{to be added}).
