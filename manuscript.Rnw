...
\item Combined, these results uncover fundamental rules in how plants are constructed and allow for systematic benchmarking of biomass partitioning routines in GVMs.
\end{enumerate}
...
The raw data in Fig.~\ref{fig:figrawdata} show a steeper increase of woody aboveground biomass ($M_S$) with plant height compared to foliage biomass ($M_F$). As a result, LMF decreased with plant height, with the three plant functional types clearly differing in LMF across nearly the entire size range (Fig.~\ref{fig:figlmf}a). When corrected for plant height by estimating LMF at a common plant height, we found that LMF was proportional to the average leaf mass per area (LMA) across the three PFTs (Fig.~\ref{fig:figlmf}b). As a consequence, LAR was invariant between PFTs, because LAR = LMF / LMA (Fig. ~\ref{fig:figlmf}c and Fig. S2). 
